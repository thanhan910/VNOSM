...
def create_geojson_object(element):
    ...
    borders = [
        [(point["lon"], point["lat"]) for point in m["geometry"]]
...
def get_each_geojson():
    INPUT_DIR = "output"
    data = {
        int(f.removesuffix(".json")): json.load(open(os.path.join(INPUT_DIR, f), "r"))
    ...
    for k, v in data.items():
        for osm_id in v["properties"]["subareas"]:
            if osm_id in data:
                ...
                print(f"[{k}] Warning: Cannot find {osm_id}")
    for k, v in data.items():
